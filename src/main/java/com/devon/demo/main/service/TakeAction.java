...
public class TakeAction implements Action {
  ...
  private String[] responseSentence = {"can you provide your pin number",
      "you are not a valid user, do you want to provide your user id again"};
  ...
  public TakeAction(Fulfillment fulfillment, Result result, String source, DummyDB dummyDB) {
    ...
    this.sapAction = (SapActionImpl) AiDemoApplication.getApplicationContext()
  ...
  public String responseToAction() {
    ...
    if (result.getAction().equalsIgnoreCase(ENTER_USER_ID_ACTION)) {
      ...
      if (result.getStringParameter(OLD_SAP_CATEGORY)
          .equals(result.getStringParameter(SAP_CATEGORY))) {
        response = "You have ask this before, please ask another question or say \"stop/cancel\" to end the conversation, thanks";
      } else {
        String sapResponse = sapAction.takeSapAction(result.getStringParameter(SAP_CATEGORY),
            result.getStringParameter(USER_ID).toLowerCase());
  ...
  private String pinValidate() {
    ...
    if (dummyDB.findUserID(result.getStringParameter(USER_ID).toLowerCase())) {
      if (dummyDB
          .findPin(result.getStringParameter(USER_ID).toLowerCase(), result.getIntParameter(PIN))) {
        sapResponse = sapAction.takeSapAction(result.getStringParameter(SAP_CATEGORY),
            result.getStringParameter(USER_ID).toLowerCase());
        response = checkSourceForSapDetailReply(sapResponse);
        resetPinContext();
  ...
  private String checkSourceForSapDetailReply(String responseMsgToSlack) {
    ...
    try {
      ...
      if (responseMsgToSlack.contains("errordetails")) {
        ...
        sapDetailRoot.getD().getResults().stream().forEach(result -> {
          strBuilder
              .append("Role: " + result.getAgrName() + "     Description: " + result.getAgrText());
          strBuilder.append("\n");
        });
      ...
      if (source != null && source.equals("slack")
          || result.getStringParameter(SAP_CATEGORY) != "role_detail") {
